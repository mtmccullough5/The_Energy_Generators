...
```{r}
totalUSSolarGen = allTableWhole %>%
  select(STATE,date_col,Solar_Gen_Total) %>%
  group_by(date_col) %>%
  summarise(US_Solar_Gen = sum(Solar_Gen_Total))

print(totalUSSolarGen)

totalUSWindGen = allTableWhole %>%
  select(STATE,date_col,Wind_Gen_Total) %>%
  group_by(date_col) %>%
  summarise(US_Wind_Gen = sum(Wind_Gen_Total))

print(totalUSWindGen)
...
##something happened and this chart looks goofy now - figure out why
